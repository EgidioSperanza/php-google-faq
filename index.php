...
$faq = [
    ...
    [
        'questionText' => 'Perché il mio account è associato a un paese?',
        'answerParagraphs' => [
        ...
        'answerDeepening' => 'Stabilire il paese associato al tuo account',
        'deepeningParagraphs' => [
    ...
    [
        ...
        'answerParagraphs' => "I risultati di ricerca di Google rispecchiano i contenuti pubblicamente disponibili sul Web. 
                I motori di ricerca non possono rimuovere i contenuti direttamente dai siti web, quindi rimuovere risultati di ricerca 
                da Google non consente di rimuovere i contenuti dal Web. Se desideri rimuovere qualcosa dal Web, devi 
                <a href=\"https://support.google.com/websearch/answer/9109?hl=it\">contattare il webmaster</a> del sito su cui sono pubblicati 
                i contenuti e chiedergli di apportare una modifica. Inoltre, se, ai sensi delle leggi europee per la protezione dei dati, 
                desideri richiedere la rimozione di determinate informazioni su di te visualizzate nei risultati di ricerca di Google,  
                <a href=\"https://support.google.com/legal/contact/lr_eudpa?product=websearch&hl=it\">fai clic qui</a>. Una volta che i contenuti 
                saranno stati rimossi e che Google avrà rilevato l'aggiornamento, le informazioni non verranno più visualizzate nei risultati di 
                ricerca di Google. In caso di una richiesta di rimozione urgente, è inoltre possibile 
    ...
    [
        ...
        'answerParagraphs' => "In alcuni casi sì. Quando fai clic su un risultato della Ricerca Google, il tuo browser web potrebbe reindirizzare 
                alla pagina web di destinazione anche l'indirizzo Internet, o URL, della pagina dei risultati di ricerca sotto forma di 
                <a href=\"https://policies.google.com/privacy/key-terms#toc-terms-referrer-url\">URL referrer.</a> Talvolta, l'URL della 
                pagina dei risultati di ricerca potrebbe contenere la query di ricerca che hai inserito. Se utilizzi la ricerca SSL 
                (la funzione di ricerca criptata di Google), nella maggior parte dei casi i termini di ricerca non vengono inviati 
                come parte dell'URL negli URL referrer. Questo comportamento può fare eccezione, ad esempio se utilizzi alcuni browser 
                meno diffusi. Ulteriori informazioni sulla ricerca SSL sono disponibili <a href=\"https://support.google.com/websearch/answer/173733\">qui</a>. Le query di ricerca o le informazioni 
                contenute nell'URL referrer potrebbero essere disponibili mediante Google Analytics o un'API 
                (Application Programming Interface). Inoltre, gli inserzionisti potrebbero ricevere informazioni relative 
...
<body>
    ...
    <?php foreach ($faq as $question) {
        foreach ($question as $keyQuestion => $valueQuestion) {
            if ($keyQuestion === 'questionText') {
                echo "<h2>$valueQuestion</h2>";
            }
            if ($keyQuestion === 'answerParagraphs') {
                if (gettype($valueQuestion) === 'string') {
                    echo "<p>$valueQuestion</p>";
                } else {
                    foreach ($valueQuestion as $answerPart) {
                        if (gettype($answerPart) === 'string') {
                            echo "<p>$answerPart</p>";
                        }
                    }
                }
            }
            if ($keyQuestion === 'answerDeepening') {
                echo "<h3>$valueQuestion</h3>";
            }
            if ($keyQuestion === 'deepeningParagraphs') {
                foreach ($valueQuestion as $deepeningValue) {
                    echo "<p>$deepeningValue</p>";
                }
            }
        }
    } ?>
